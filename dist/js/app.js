(() => {
            ...
            function setSpollerAction(e) {
                ...
                if (el.closest("[data-spoller]")) {
                    ...
                    if (!spollersBlock.querySelectorAll("._slide").length) {
                        ...
                        if (spollerTitle.nextElementSibling.firstChild.hasAttribute("data-tabs-item")) spollerTitle.nextElementSibling.firstChild.hidden = false;
    ...
    function tabs() {
        ...
        function initTabs(tabsBlock) {
            let tabsTitles = tabsBlock.querySelectorAll("[data-tabs-titles] [data-tab]");
    ...
    function propertyCategorySlider() {
        const sliderBlocks = document.querySelectorAll('[data-slider="property-category"]');
        if (sliderBlocks.length) sliderBlocks.forEach((sliderBlock => {
    ...
    function similarProperties() {
        const sliderBlocks = document.querySelectorAll('[data-slider="similar-properties"]');
        if (sliderBlocks.length) sliderBlocks.forEach((sliderBlock => {
            const slider = sliderBlock.querySelector(`[data-slider]`);
            const sliderPagination = sliderBlock.querySelector("[data-slider-pagination]");
            const btnNext = sliderBlock.querySelector("[data-slider-nextbtn]");
    ...
    function propertySlider() {
        const slidersBlock = document.querySelector('[data-sliders="property-slider"]');
        if (slidersBlock) {
            const slider = slidersBlock.querySelector('[data-slider="about-property-slider"]');
            const sliderThumb = slidersBlock.querySelector('[data-slider="about-property-slider-thumb"]');
            const btnNext = slidersBlock.querySelector("[data-slider-nextbtn]");
            const btnPrev = slidersBlock.querySelector("[data-slider-prevbtn]");
    ...
    function gallerySlider() {
        const sliderBlock = document.querySelector('[data-slider="gallery-slider"]');
        if (sliderBlock) {
            const slider = sliderBlock.querySelector(`[data-slider]`);
            const btnNext = sliderBlock.querySelector("[data-slider-nextbtn]");
            const btnPrev = sliderBlock.querySelector("[data-slider-prevbtn]");
    ...
    toggleBtn();
    function openHideSearchForm(e) {
        if (e.target.closest(".search")) {
            open();
            if (e.target.closest(".search__title")) {
                const clickedButtonValue = e.target.closest(".search__title").querySelector("span").textContent.toLowerCase();
                const searchWrapper = document.querySelector(".search__wrapper");
                const tabs = searchWrapper.querySelectorAll(".search__title");
                tabs.forEach((tab => {
                    if (tab.querySelector("span") && tab.querySelector("span").textContent.toLowerCase() === clickedButtonValue) tab.click();
                }));
            }
        } else if (e.target.closest("[data-open-search]")) open(); else close();
        if (e.target.closest("[data-close-search]")) close();
        function close() {
            if (document.documentElement.classList.contains("_search-open")) document.documentElement.classList.remove("_search-open");
        }
        function open() {
            document.documentElement.classList.add("_search-open");
        }
    }
    function checkRadioLocation(e) {
        if (e.target.closest("[data-radio-location]")) {
            const activeButton = document.querySelector("[data-radio-location]._active");
            activeButton ? activeButton.classList.remove("_active") : null;
            const button = e.target.closest("[data-radio-location]");
    ...
    const allGalleries = document.querySelectorAll("[data-gallery-wrapper]");
    if (allGalleries.length) allGalleries.forEach((gallery => {
        Oe.bind(gallery, '[data-fancybox="gallery"]', {
    ...
    function checkboxRadioChecked() {
        window.addEventListener("click", (e => {
            if (e.target.closest(".checkbox-item") || e.target.closest(".radio")) {
                let input = null;
                if (e.target.closest(".checkbox-item")) {
                    input = e.target.closest(".checkbox-item").querySelector('input[type="checkbox"]');
        ...
        window.addEventListener("keydown", (e => {
            if (e.key === "Enter") {
                ...
                if (focusedElement.closest(".checkbox-item") || focusedElement.closest(".radio")) {
                    let input = null;
                    if (focusedElement.closest(".checkbox-item")) {
                        input = focusedElement.closest(".checkbox-item").querySelector('input[type="checkbox"]');
                        input ? input.checked = !input.checked : null;
                    }
                    let event = new Event("inputChange");
                    input ? input.dispatchEvent(event) : null;
